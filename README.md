...
面向小红书运营者和创作者的研究、写作与审校 Skill，覆盖类目调研、账号拆解、选题成稿、发布诊断、评论边界和商业承接。
...
它目前由一份可安装 Skill、七份 reference、13 份落库模板和一个验证器组成。适合完成这些工作：
...
flowchart LR
    ...
    B --> C["近期基线、模式与反例"]
    C --> D["topic<br/>研究问题 / 实验选题 / active"]
    D --> E["draft<br/>标题、封面、完整成稿"]
    E --> F["合规审校 + 创意审校"]
    F --> G["发布与后台数据"]
    G --> H["refresh<br/>增量复盘"]
    H --> D
...
下面故意用成人亲密关系这个高风险场景举例，因为它会同时触发调研、真实性、敏感规则和商业边界。换成家居、旅行、美妆或职场类目，输入合同不变。

```text
使用 redbook-writing，做一次 discovery。

类目：成人亲密关系
目标读者：25–35 岁，正在建立稳定关系的女性
具体处境：不知道怎样和伴侣讨论边界、需求和安全感
账号目标：先建立信任，后续可能承接女性向成人用品
可用材料：公开笔记和评论；没有后台数据
生产条件：不出镜；每周最多完成 3 篇轮播
商业目标：当前不卖货，不设计购买 CTA
禁区：伪真人、假聊天、未授权截图、竞品评论截流
交付：对标账号、近期笔记、内容模式、反例和实验选题
...
选择一个 primary_job 为 relationship_building 的题目。
...
3. 完整轮播逐页文案
4. 真实性与商业关系披露
5. compliance review 和 creative review
6. 发布后的主指标与下一轮单变量
```

如果 topic 没有足够证据，`draft` 会先补最小必要调研，不会直接把猜测写成成稿。
